...
- **S1 — Flashcard Decks** 🔄 RE-SCOPED → FUTURE PLAN (decision 2026-07-23): author flashcards as STATIC VERIFIED DATA FILES, NOT runtime generation. Rationale: flashcards are static facts (no {{placeholder}} randomization like question templates), so there's no reason to generate them — authoring in-repo gives verifiable accuracy (reviewable in git, no hallucination/fact-check needed), permanence, editability, and the data file IS the "table of what we need" (100% coverage by construction). Plan: (1) author `src/data/flashcards/<cert>.js` per cert in reviewable batches — concept cards mapped to examObjectives sub-objectives + CompTIA acronym decks (Sec+/Net+) + CCNA commands/terminology deck; each card {front, back, example}; (2) delete the 201 legacy AI cards; (3) one-time seed into flashcards table (keeps existing study UI + flashcard_progress); (4) RETIRE generate-flashcards route. NO deck/card_key migration needed (data files carry structure). Existing 201 AI cards stay studyable until replaced. Start with ONE deck as a quality sample before authoring hundreds.
...
- **S4 — Coverage Engine (R1)** ✅ BUILT (Phase 105) — `src/data/examObjectives.js` (current-version sub-objective lists, all 3 certs) + `sub_objective TEXT` column + generator targets UNCOVERED sub-objectives + coverage table UI on templates page (X/Y per domain).
  - **"EVEN / CORRECT SPACING" — the BETTER way (user asked 2026-07-22):** don't rely on the owner remembering to generate per domain. Build a coverage-aware generation orchestrator:
    ...
    3. **"Fill Gaps" button:** computes which domain×difficulty combos are below target and auto-generates the needed batches SEQUENTIALLY with a progress bar (respects the batch-size-5 limit + verify pass). One click → pool fills to correct weighted coverage. Subsumes manual per-domain generation.
  - NOTE: template variable_sets mean each template yields many varied concrete questions, so pool size need not be huge — but weighted coverage + sub-objective breadth is what kills the "brand new question" feeling. Freshness comes from breadth, not just count.
- **S5 — Pool Cleanup + Regeneration** ⬜ — audit existing 161 (all-hard, off-style); regenerate medium+hard per domain with style guides + verify + coverage. Content fidelity complete after this. (gap #10)
  - **FK SAFETY VERIFIED (2026-07-22):** only FK to question_templates is `flagged_questions.template_id` with `ON DELETE SET NULL` (0 flags exist). question_answers + topic_performance have NO FK (snapshot/aggregate). → HARD DELETE of all templates is SAFE: no failure, no cascade, answer history + weakness tracking survive intact. Reset the pool freely.
  - **DELETE/RESET TOOL (user requested 2026-07-22):** owner-only controls on templates page (or premade-templates): "Delete all templates for [cert]" and "Delete all templates (all certs)" + per domain/difficulty. HARD delete (not retire) since goal is fresh start. Double-confirm modal (⚠️ irreversible, shows count). New reset scope in /api/reset or a dedicated owner endpoint with owner-email guard.
...
4. **No-going-back navigation.** DECIDED (user 2026-07-22): Real Exam mode = no going back for ALL certs. Once you advance, the question locks. Remove Previous + block returning to answered questions in real mode only; Practice/Simulation keep free navigation. (Note: real CompTIA technically allows review, but user wants the stricter no-back for all — matches Cisco and maximizes the "no shock" prep value.)
...
- **RLS write sweep (all public tables):** only two other tables have permissive (`true`) write policies — `ai_food_intel_cache` and `nutrient_profiles`. Both are **intentional shared caches** (documented as open SELECT/INSERT/UPDATE so any user can populate AI-generated cache). Noted as a minor cache-poisoning surface if the user base grows beyond trusted invitees; not changed. `question_templates` (Phase 123) was the only unintended open-write.
- **Study Hub audit (items 1–12) complete.** Tally: 6 functional bugs fixed (fillTemplate crash, progress streak TZ, flashcards ×2, study-mode types) + 1 security fix (question_templates RLS) + 4 earlier test-page fixes (Real Exam crash, timed-out empty-save, array user_answer, pass-estimate bg) + pacing gate. Noted-not-fixed: CLI duplicate-goal edge; shared-cache poisoning surface.
- Next (second pass): Life Hub (nutrition/water/workouts/brief/health).
