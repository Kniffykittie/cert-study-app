...
- `BarcodeScannerModal` component (`src/components/BarcodeScannerModal.js`) — camera modal using native BarcodeDetector API; falls back to `barcode-detector/pure` (ZXing WASM) on iOS Safari; supports EAN-13/8, UPC-A/E, Code-128/39; requires same barcode detected 3 consecutive frames to prevent accidental scans; uses onResultRef pattern so effect runs exactly once (inline onResult prop would have restarted the camera stream on every render, crashing the tab); `barcode-detector` npm package added to package.json
