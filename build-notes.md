...
- **Travel / bodyweight-only mode** — one-tap toggle on the workout plan page: "✈️ Traveling this week." Switches the active plan to a bodyweight-only version of the same workout structure for the current week only. Does not overwrite the saved plan — reverts automatically after 7 days or when the user toggles it off. Generates the travel version on demand using the existing generate-plan route with equipment forced to bodyweight only.
