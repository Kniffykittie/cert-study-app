...
| 2G | ✅ Done | My Favorites sorted by recency — `last_logged_at` + `log_count` columns added to `my_foods`; `bump_my_food_recency` Postgres function increments both on every log; GET route orders by last_logged_at desc → log_count desc → name; optimistic re-sort in client state on log; log count badge (×N) and "logged today / yesterday / Nd ago" sub-label on each favorite row. |
