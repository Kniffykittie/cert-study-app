...
src/
  ...
  supabase/
    ...
    study/AnswerArea.js                Shared answer-rendering surface for the test flow — dispatches on question_type (mc/multi/ordering/matching); one component for all 4 states (practice-reveal/simulation/real/results-review) via revealed + explanationScope + verboseMarks props; ordering=tap-to-sequence, matching=lettered picker, both show rationale on reveal; new question types add a branch here
...
| `question_templates` | Template library with variable_sets and is_retired flag; `exhibit` JSONB (topology/config), `question_type` TEXT default 'mc' (mc/multi/ordering/matching), `correct_answers` TEXT[] (multi, null=single), `correct_answer` nullable, `type_payload` JSONB (ordering items / matching terms+defs), `rationale` TEXT (PBQ explanation) |
