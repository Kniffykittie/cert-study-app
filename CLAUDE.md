...
src/
  app/
    api/
      ...
      checkin/
        insight/route.js               POST — Haiku check-in insight (2 sentences + proposed_actions); rate-limited 2/day (checkin-insight-YYYY-MM-DD); saves to daily_checkins (morning or afternoon fields); SORE_CONFLICT_MAP checks planned exercises vs sore spots; coach_memory_context from client (context snapshot — not re-fetched); note wrapped in user_input tags; uses getUser() + is_disabled check
        chat/route.js                  POST — Haiku Keep Talking multi-turn; rate-limited 24 turns/day (checkin-chat-YYYY-MM-DD); receives frozen contextSnapshot + messages[]; no DB re-fetch; returns { message, proposed_actions[] }; uses getUser() + is_disabled check
      workouts/
        ...
        day-hub/route.js               GET ?date=YYYY-MM-DD — returns plan_day, workout_log, workout_sets, stretch_logs (with context), prev_session hints; date format validated; getUser() only (no AI)
      ...
      life-hub/
        daily-brief/route.js           GET ?window=(morning|afternoon|evening) returns cached brief for that window; POST accepts window in body — morning gathers 10+ tables + calls Claude (reads my_week + schedule_events for today's schedule/events, meal times, workout time with supplement timing alignment; injects today's + 14-day-upcoming events); evening gathers today's data (past-tense) but is READINESS-GATED: first gen held until max(bedtime−60, last schedule_events end today) in local tz, else returns { notReady, readyAt }; POST { refresh:true } bypasses gate + regenerates; afternoon is a side effect of checkin/insight; uses getUser() + is_disabled check
        my-week/route.js               GET ?week=YYYY-MM-DD returns all 7 rows for that week_start (Monday); POST { week, day: { day_of_week, day_type, meal times, workout_time, commitments, day_notes } } upserts one day row, syncs day_type back to goals_profiles.weekly_schedule for backward compat; getUser() only (no AI)
        schedule-events/route.js       GET ?month=YYYY-MM returns { recurring (all weekly), oneoff (dated within month) }; POST create/update (title/category/start_time/end_time/notes + recurrence 'weekly'→day_of_week or 'once'→event_date); DELETE ?id= (all scoped .eq user_id); getUser() only
        schedule-notes/route.js        GET ?scope=month|week&period= (YYYY-MM or Monday date) returns note; POST upserts; getUser() only
    ...
    life-hub/
      ...
      page.js                          Life Hub landing — "Today's Schedule" card (today's schedule_events, below status pills, links to My Schedule); Zone 1 = 4-pill status bar (calories/workouts/steps/water, section-colored); Recovery Score (5-component 0–100, above briefs); Zone 2 = Three Daily Brief cards (morning=purple always visible, afternoon=yellow shown when check-in insight exists, evening=indigo shown after 6pm or when generated; each collapsible, color-coded); Check-In card (directly below brief — Morning/Afternoon tabs; Morning = full form with energy/mood/sleep/note/save + 28-day heatmap; Afternoon tab = read-only display of afternoon_energy/mood/note, shown only when afternoon data exists); Zone 3 = 2×2 live section summary cards (Nutrition/Workouts/Health/Goals); visibilitychange listener refreshes kcal on tab focus without a full reload; card-type system (S14, Phase 135): Recovery = filled score-tint hero card, Brief + Check-In = plain full-border panels, status pills + section cards keep the 3px left-accent language — don't collapse these back to a single uniform border style
      my-week/page.js                  My Schedule — MONTHLY calendar (Mon-first grid, event dots/chips per day, day-type colored top border, today highlighted); tap a day → DayDetail modal (timed events + daily routine). Events = `schedule_events`: recurring weekly blocks (work hours, standing commitments — set once, repeat every week) OR one-off dated events (concert on a real date); each has title/category(work/social/appointment/travel/other)/start-end time/notes. Routine still uses `my_week` (day_type pills + meal/workout times) edited inline in the modal. Month-level notes (`schedule_notes`). Feeds Daily Brief (today's events + 14-day upcoming) and AI routes. Route path kept /life-hub/my-week
      ...
      workouts/
        page.js                        My Workout Plan — weekly plan cards sorted Mon-Sun, day reassignment, add/remove exercises with AI check-in, add/change cardio on rest days; "Open →" link per card to Day Hub; weekly completion bar (X/Y workouts); Start Workout / ▶ Resume Workout / ✓ Done Today button logic per day; Add Exercise modal grouped by muscle group with ? detail popup; gates on goals profile
        ...
        exercises/page.js              Exercise Library — sticky muscle-group nav, scrollable grouped sections, image cards, detail modal with form cues + PR chip (🏆 max working weight ever for that exercise), Cardio section
        ...
        history/page.js                Workout history — week-grouped format (8 weeks per page, Load More); "View Day →" links to Day Hub read-only; expandable set chips by exercise
    ...
    study-hub/
      ...
      test/page.js                     Take a Test (practice / simulation / real exam); Real Exam mode = all domains forced + official weights (no spaced-rep personalization) + medium+hard difficulty blend (not hard-only) + PBQ/CLI questions front-loaded + NO going back (linear, must answer to advance) + honest pass-likelihood estimate + pacing feedback (avg time/question vs real exam budget) on results (Real Exam mode only); Mixed mode excludes CLI (CCNA-specific); question types mc/multi/ordering/matching/cli via AnswerArea
  ...
  data/
    ...
    stretches.js                       38 stretches across 10 muscle groups; exports STRETCHES, STRETCH_MUSCLE_GROUPS, BODY_PART_TO_STRETCH_GROUPS, STRETCH_BY_ID, STRETCH_BY_GROUP, SORE_SPOT_TO_MUSCLE_GROUP, getRecommendedStretches(bodyParts, soreSpots), getTimingLabel(ideal_timing); each stretch: id, name, muscle_group, stretch_type (dynamic/static/both), ideal_timing (pre_workout/post_workout_or_bed/anytime), how_to, why (2–3 sentences), common_mistakes, contraindications, duration_seconds
  components/
    nutrition/
    ...
    ThemePicker.js                     Settings → 🎨 Appearance — swatch-card picker for 9 color themes (Villainous default / Midnight / Amethyst / Slate / Rose Gold / Acid / Sunshine / Bubblegum); live-applies via `data-theme` on <html> + persists to localStorage `theme_preset`; exports THEME_PRESETS. Themes override only chrome+accent CSS vars (in globals.css `:root[data-theme=...]` blocks); success/error/warning stay constant. Backgrounds are chosen by color theory to flatter each accent (complementary/harmonious, or neutral charcoal where a complement would fight) — NOT a darkened version of the accent (Phase 139). No-flash inline script in layout.js head stamps the theme before first paint. Life Hub section colors ARE theme-driven (Phase 137) via `src/lib/sectionColors.js` `useSectionColors()` — per-theme SECTION_PALETTES, hex strings so `${color}22` tinting still works; ThemePicker dispatches a `themechange` event for live recolor. Waves 1–3a done = landing + sidebar + bottom nav + 4 section landing pages + measurements/my-week/wraps/health-subpages; functionally complete — chrome + all section landings + major sub-pages recolor. Intentionally left fixed (semantic category palettes, not section identity): stretches type/timing colors, CheckInSheet + brief time-of-day colors, nutrition modal category colors.
    MfaGate.js                         App-wide (root layout) — redirects a signed-in-but-aal1-while-2FA-enrolled session to /login to complete the code; accounts without 2FA unaffected; skips public paths. UX half of 2FA enforcement (DB RLS aal2 rule is the real lock)
    TimezoneSync.js                    App-wide (root layout) — on each app open, updates profiles.timezone to the device's current IANA tz so server-side notifications auto-follow travel; writes only when changed
    (Settings → Notifications): "Wake & Bedtime" card anchors the DEFAULT send time for all 10 notification types; each notification row has its own optional time picker (notification_times override) + ⓘ explainer; no duplicate brief-time preview
    StudyHubSidebar.js                 Nav sidebar with test-in-progress guard
    StudyHubBottomNav.js               Mobile-only bottom tab bar (Home/Test/Cards/Labs/Progress); mirrors LifeHubBottomNav; carries test-in-progress leave-guard; mounted in study-hub/layout.js
    LifeHubSidebar.js                  Life Hub nav — section color system (overview=purple, health=green, nutrition=orange, workouts=blue, goals=teal); Overview section (Dashboard + Monthly Wrap), Goals dropdown (Overview + Measurements + Setup), Health dropdown (Overview + Step Tracker + Heart Rate + Sleep Tracker), Nutrition dropdown (Food Log + Meal Plan + Encyclopedia + Hydration + Supplements), Workouts dropdown (My Plan + History + Exercise Library + Stretching & Mobility + Stretch Library); Hydration and Supplements live under Nutrition group; auto-opens on active routes; SECTION_COLORS constant defines all section accent colors; nav links/dropdowns use 9px vertical padding for tap targets (S14 typography pass)
...
| `profiles` | User display name, exam_dates JSONB, daily_goal INT, default_cert TEXT, is_disabled BOOLEAN (owner ban flag, checked in every AI route), settings_pin_hash TEXT (bcrypt hash for Settings page Privacy PIN), authenticator_name TEXT (e.g. "Google Authenticator" — shown on 2FA login screen), notification_preferences JSONB (10 boolean keys: morning_brief/midday_checkin/evening_wrap/workout_reminder/hydration_nudge/study_streak/supplement_reminder/weigh_in_reminder/body_measurement_reminder/wrap_ready; briefs default true, nudges default false), notification_times JSONB (per-type "HH:MM" custom send-time overrides; unset key = derived default from wake/bedtime), timezone TEXT (IANA tz — Edge Function fires notifications at the user's LOCAL time) |
...
| `stretch_logs` | Stretch session logs — user_id, date, stretch_ids TEXT[], session_type (pre_workout/post_workout/standalone), context TEXT CHECK IN ('pre_workout','post_workout','bedtime','standalone'), duration_seconds, logged_at; RLS user-scoped |
| `workout_session_overrides` | Today-only exercise swaps from check-in suggestions — user_id, date, original_exercise, override_exercise, reason, applied_at; RLS user-scoped; workout log reads on mount to substitute exercise names; plan record never modified |
| `daily_briefs` | Cached AI daily brief — brief_text, data_snapshot JSONB, window TEXT (morning/afternoon/evening); UNIQUE on user_id + date + window; morning generated on first Life Hub visit; afternoon upserted as side effect of check-in insight; evening generated on first Life Hub visit after 6pm; RLS user-scoped |
| `my_week` | Per-user per-week daily planner (ROUTINE layer for My Schedule) — week_start DATE (always Monday), day_of_week SMALLINT (0=Mon…6=Sun), day_type, breakfast/lunch/dinner time, workout_time, workout_duration_min, commitments TEXT, day_notes TEXT; UNIQUE on user_id+week_start+day_of_week; RLS user-scoped; day_type synced back to goals_profiles.weekly_schedule on save |
| `schedule_events` | Timed schedule blocks for My Schedule — title, category (work/social/appointment/travel/other), start_time/end_time TIME, notes; recurrence 'weekly' (day_of_week 0–6, repeats every week — work hours/standing commitments) OR 'once' (event_date DATE — one-off like a concert); CHECK enforces the right anchor per recurrence; RLS user-scoped |
| `schedule_notes` | Free-text planning notes for My Schedule — scope ('month'|'week'), period (YYYY-MM or Monday date), note TEXT; UNIQUE on user_id+scope+period; RLS user-scoped |
...
## Gotchas
- **A restrictive `require_aal2_if_enrolled` RLS policy sits on 45 personal-data tables** (Phase 133) — `using ((select auth.jwt()->>'aal') = 'aal2' or not public.has_verified_mfa())`. This is the real 2FA lock. Any NEW personal-data table with a `user_id` column must get this same policy in its migration, or a 2FA-enrolled-but-aal1 session could reach it. Excluded on purpose: `recovery_codes` (escape hatch), `api_rate_limits` (infra), `flashcards` (shared). Never DROP the `has_verified_mfa()` helper — the policies depend on it.
...
- **Wrap user free text for AI prompts with `wrapUserInput()` from `src/lib/aiSafety.js`**, not a bare `<user_input>${x}</user_input>` — the helper strips `</user_input>` from the content so a user can't break out of the data envelope. Use `sanitizeForPrompt()` for untagged context fields the client echoes back. **All AI routes now use it** — never reintroduce a bare `<user_input>${x}</user_input>`.
- **Shared caches are read-only for clients; write them via `createAdminClient()` (service role) from `src/lib/supabaseAdmin.js`** — `food_cache`, `ai_food_intel_cache`, `nutrient_profiles`, `supplement_profiles` are locked (SELECT open, no client write policies). The generating route must write via the admin client or the cache silently won't populate. `exercises`/`stretches` are read-only reference tables (seed via migration/admin only).
