  ...
  app/
    ...
    life-hub/
      ...
      workouts/
        ...
        stretches/page.js             Stretch Reference / Guided Session — when accessed with `?context=&ids=` params from Day Hub, shows only the recommended stretch IDs in order with all pre-checked (uncheck to indicate skipped), hides type/muscle filters, shows per-stretch WHY text; without params shows all 38 stretches with filter by type/muscle group; expandable rows with how-to, mistake warnings, contraindications
      nutrition/
        add-food/page.js               Standalone add-food page — Favorites tab with sub-tab pills (🌟 All | 🍽️ Foods & Meals | 🥤 Drinks | 🍿 Snacks | 🥚 Ingredients; smart default per slot; localStorage persisted; count badges; filter works within active sub-tab) + Search tab (OFFs + barcode + AI estimate); navigates here from all "Add [slot]" buttons; always sends `date: toLocaleDateString('en-CA')` (local timezone) in POST body; logs food then returns to nutrition
