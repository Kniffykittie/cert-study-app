...
| `my_foods` | User's personal saved food library — name, brand, serving_size_label, full macro + micronutrient fields, last_logged_at TIMESTAMPTZ, log_count INT; sorted by recency on GET; `bump_my_food_recency(food_id, uid)` Postgres function increments both on every food log; RLS user-scoped |
