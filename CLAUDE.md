...
src/
  ...
  components/
    ...
    ThemePicker.js                     Settings → 🎨 Appearance — swatch-card picker for 9 color themes (Villainous default / Midnight / Amethyst / Slate / Rose Gold / Acid / Sunshine / Bubblegum); live-applies via `data-theme` on <html> + persists to localStorage `theme_preset`; exports THEME_PRESETS. Themes override only chrome+accent CSS vars (in globals.css `:root[data-theme=...]` blocks); success/error/warning stay constant. No-flash inline script in layout.js head stamps the theme before first paint. Life Hub section colors ARE theme-driven (Phase 137) via `src/lib/sectionColors.js` `useSectionColors()` — per-theme SECTION_PALETTES, hex strings so `${color}22` tinting still works; ThemePicker dispatches a `themechange` event for live recolor. Waves 1–2 done = landing + sidebar + bottom nav + the 4 section landing pages (nutrition/workouts/health/goals); remaining deeper sub-pages still use local hardcoded hexes (waves 3+).
