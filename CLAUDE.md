...
| `my_foods` | User's personal saved food library — name, brand, serving_size_label, full macro + micronutrient fields, last_logged_at TIMESTAMPTZ, log_count INT, is_pinned BOOLEAN; sorted by is_pinned desc → last_logged_at desc → log_count desc → name on GET; `bump_my_food_recency(food_id, uid)` Postgres function increments log_count + sets last_logged_at on every food log; PATCH route toggles is_pinned; RLS user-scoped |
